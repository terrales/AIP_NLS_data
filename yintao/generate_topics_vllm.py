"""
Generate 3-layered topics from vllm embeddings.
Uses embeddings from embeddings2/embeddings_vllm_float32.pkl

Features:
- Runs on GPU 1 (set via CUDA_VISIBLE_DEVICES)
- Saves UMAP checkpoint after dimension reduction
- Can resume from UMAP checkpoint if script fails
- Slack notifications for progress

Pipeline:
1. Load embeddings (or UMAP checkpoint if available) and reduce dimensions with UMAP → save reduced embeddings
...
def send_slack_notification(message, is_error=False):
    ...
    payload = {
        'text': f"{emoji} *[VLLM/GPU1] Topic Generation Update* ({timestamp})\n{message}"
    }
...
# Configuration - VLLM version (different output files to avoid overwriting)
EMBEDDINGS_PATH = '/disk/scratch/s1891075/AIP_NLS_data/yintao/embeddings2/embeddings_vllm_float32.pkl'
...
# Intermediate output paths (unique names to avoid overwriting)
REDUCED_EMBEDDINGS_PATH = os.path.join(OUTPUT_DIR, 'reduced_embeddings_vllm.pkl')
TOPIC_0_PATH = os.path.join(OUTPUT_DIR, 'topic_0_vllm.parquet')
TOPIC_1_PATH = os.path.join(OUTPUT_DIR, 'topic_1_vllm.parquet')
FINAL_OUTPUT_PATH = os.path.join(OUTPUT_DIR, 'layer_3_topics_vllm.csv')
...
def cluster_and_name_topics(topic_table, n_neighbors, n_components, min_cluster_size, layer_name):
    ...
    # Assign noise points
    labels_assigned = assign_noise_pytorch(reduced_data, labels, noise_batch_size=512, clean_batch_size=50000)
...
def main(resume_from_umap=False):
    try:
        log_and_notify("🚀 Starting topic generation with VLLM embeddings (GPU 1)")
        ...
        if resume_from_umap and os.path.exists(REDUCED_EMBEDDINGS_PATH):
            ...
            # Save reduced embeddings checkpoint immediately after UMAP
            print(f"Saving UMAP checkpoint to {REDUCED_EMBEDDINGS_PATH}...")
            ...
            print("UMAP checkpoint saved!")

            log_and_notify(f"✅ UMAP complete! Reduced shape: {reduced_embeddings.shape}. Checkpoint saved to {REDUCED_EMBEDDINGS_PATH}")
...
Topic name:"""
        ...
        print("Summary")
        print("="*60)
        summary = f"""🎉 *[VLLM/GPU1] Topic Generation Complete!*
...
• Final output: {FINAL_OUTPUT_PATH}"""
        ...
        error_msg = f"❌ [VLLM/GPU1] Error occurred: {str(e)}\n\nTraceback:\n{traceback.format_exc()}"
        log_and_notify(error_msg, is_error=True)
        raise


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate topics from VLLM embeddings')
