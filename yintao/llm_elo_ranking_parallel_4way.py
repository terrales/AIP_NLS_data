"""
Parallel LLM-based Elo Rating System for 4-Way Topic Comparison
Compares topic_0 from 3 modeling methods + original subject field
Only uses rows where subject field is not null
"""
import os
import json
import random
...
class ParallelEloRankingSystem4Way:
    """Parallel LLM-based Elo ranking system for 4-way topic comparison."""
    ...
    def __init__(self, settings_config: Dict[str, Any], api_key: str = None,
        ...
        self.data_cache = {}
        self.valid_indices = None  # Indices where subject is not null
        self.votes = []
    ...
    def _load_all_data(self):
        """Load data for all settings and filter to rows with subject."""
        ...
        # Find valid indices (rows where subject is not null across all datasets)
        print("\nFiltering to rows with subject field...")
        valid_mask = None

        for setting_id in self.setting_ids:
            df = self.data_cache[setting_id]
            if 'subject' in df.columns:
                current_mask = df['subject'].notna() & (df['subject'].astype(str).str.strip() != '')
        ...
        if valid_mask is not None:
            ...
            print(f"  Found {len(self.valid_indices)} rows with valid subject field")
        else:
            raise ValueError("No subject field found in any dataset")
    ...
    def get_book_topic_0(self, setting_id: str, book_index: int) -> str:
        ...
        # For "subject" setting, return the subject field
        if setting_id == 'subject':
            if 'subject' in df.columns:
                val = row['subject']
                return val if pd.notna(val) else "N/A"
            return "N/A"

        # For other settings, return topic_0
    ...
    def get_book_info(self, setting_id: str, book_index: int) -> Dict[str, str]:
        """Get book metadata (WITHOUT subject - it's now a competing system)."""
        df = self.data_cache[setting_id]
        if book_index >= len(df):
            return None
        ...
        # Include key metadata columns (excluding subject)
        metadata_cols = ['title', 'creator', 'description']
    ...
    def format_comparison_prompt(self, book_info: Dict, topic_0_a: str, topic_0_b: str,
        ...
        # Format topic from both settings
        topics_text = f"""
Setting A ({setting_a_name}):
  {topic_0_a}

Setting B ({setting_b_name}):
  {topic_0_b}
"""

        prompt = f"""You are evaluating two different topic categorization approaches for library books.
Each approach assigns a topic/subject category to books.

{book_text}
{topics_text}

Please evaluate which setting provides a better topic categorization for this book. Consider:
1. Relevance: Does the topic accurately reflect the book's main content?
2. Specificity: Is the topic appropriately specific and useful?
3. Discoverability: Would this topic help users find or browse for this book?
4. Clarity: Is the topic clear and understandable?
...
Your response:"""
    ...
    async def ask_llm_judge(self, book_info: Dict, topic_0_a: str, topic_0_b: str,
                           setting_a_name: str, setting_b_name: str) -> str:
        """Ask LLM to judge which topic assignment is better (async)."""
    ...
    async def run_single_vote(self) -> Dict[str, Any]:
        ...
        # Get random valid book index (from pre-filtered list)
        book_index = random.choice(self.valid_indices)
    ...
    async def run_tournament(self, num_votes: int, batch_size: int = 100) -> Dict[str, Any]:
        ...
        """
        print(f"\n{'='*60}")
        print(f"Parallel LLM Elo Ranking Tournament (4-Way Comparison)")
        print(f"Model: {self.model}")
        print(f"Settings to compare: {len(self.settings)}")
        print(f"Number of votes: {num_votes}")
        print(f"Valid rows with subject: {len(self.valid_indices)}")
        ...
        return {
            ...
            'valid_rows': len(self.valid_indices),
            'votes': self.votes,
            'model': self.model,
            'timestamp': datetime.now().isoformat(),
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        print(f"\n{'='*80}")
        print(f"FINAL RANKINGS (4-Way Topic Comparison)")
        print(f"{'='*80}\n")
        ...
        print(f"Valid rows with subject: {results['valid_rows']}")
...
async def main_async(args):
    ...
    ranking_system = ParallelEloRankingSystem4Way(
...
def main():
    parser = argparse.ArgumentParser(
        description='Parallel LLM-based Elo ranking system for 4-way topic comparison'
    )
    parser.add_argument(
        '--config',
        type=str,
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='topic_4way_comparison_results.json',
        help='Output file for results (default: topic_4way_comparison_results.json)'
    )
    parser.add_argument(
        '--api-key',
        type=str,
