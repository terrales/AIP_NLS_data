"""
Generate 3-layered topics from embeddings.
Uses the new embeddings from embeddings2/embeddings_vllm.pkl
...
# Configuration
EMBEDDINGS_PATH = '/disk/scratch/s1891075/AIP_NLS_data/yintao/embeddings2/embeddings_vllm.pkl'
...
EMBEDDINGS_FLOAT32_PATH = os.path.join(OUTPUT_DIR, 'embeddings2/embeddings_vllm_float32.pkl')
REDUCED_EMBEDDINGS_PATH = os.path.join(OUTPUT_DIR, 'reduced_embeddings_new.pkl')
TOPIC_0_PATH = os.path.join(OUTPUT_DIR, 'topic_0_new.parquet')
TOPIC_1_PATH = os.path.join(OUTPUT_DIR, 'topic_1_new.parquet')
FINAL_OUTPUT_PATH = os.path.join(OUTPUT_DIR, 'layer_3_topics_all_2.csv')


def assign_noise_pytorch(data, labels, batch_size=4096, device='cuda'):
    ...
    for i in tqdm(list(range(0, num_noise, batch_size))):
        end = min(i + batch_size, num_noise)
        noise_batch = noise_data[i:end]
        dists = torch.cdist(noise_batch, clean_data, p=2)
        min_dist_indices = torch.argmin(dists, dim=1)
        closest_labels = clean_labels[min_dist_indices]
        original_indices_for_batch = noise_indices[i:end]
        new_labels[original_indices_for_batch] = closest_labels
...
def main():
    ...
    labels_assigned = assign_noise_pytorch(reduced_embeddings, labels, batch_size=8192)
...
Topic name:"""
    ...
    print(f"Unique topic_2: {data['topic_2'].nunique()}")
    print(f"\nOutput files:")
    print(f"  - Reduced embeddings: {REDUCED_EMBEDDINGS_PATH}")
    print(f"  - Topic_0 data: {TOPIC_0_PATH}")
    print(f"  - Topic_1 data: {TOPIC_1_PATH}")
